...
public class AccountsCatchupHostedService : BackgroundService, IAccountsCatchupHostedService
{
    ...
    {
        return _catchupSubscription.StartAsync(AccountsDomainStreamNames.AllAccounts, "AccountsCatchupHostedService", cancellationToken,
            (eventWrapper, ct) =>
            {
    ...
    private void HandleEvent(AccountOpenedEvent_v1 @event, IEventWrapper eventWrapper)
    {
        ...
        {
            _logger.LogWarning($"eventData is null or missing or malformed, ignoring event {AccountsDomainStreamNames.AllAccounts}#{eventWrapper.EventNumber}");
        }
    ...
    private void HandleEvent(AccountStatusUpdated_v1 @event, IEventWrapper eventWrapper)
    {
        if (@event is null || @event.Id == Guid.Empty)
        {
            _logger.LogWarning($"eventData is null or missing a sanctioned name to add, ignoring event {AccountsDomainStreamNames.AllAccounts}#{eventWrapper.EventNumber}");
        }
